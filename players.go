...
func (i *PlayerService) BattleLog() (Battles, error) {
	path := "/v1/players/%s/battlelog"
	url := fmt.Sprintf(path, NormaliseTag(i.tag))
	req, err := i.c.NewRequest("GET", url, nil)
	var list Battles

	if err == nil {
		_, err = i.c.Do(req, &list, path)
	}
